...
@WebServlet(name = "TipoChallengueService", urlPatterns = {"/TipoChallengueService"}) 
public class TipoChallengueService extends HttpServlet { 
 
    public static TipoChallengueI str = new TipoChallengueI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accTipoChallengue").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accTipoChallengue").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accTipoChallengue").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accTipoChallengue").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de TipoChallengue " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMTipoChallengue").equals("n")){ 
            ...
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de TipoChallengue", "TipoChallengue", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
    ...
    public static String eliminar(HttpServletRequest request){ 
        try{ 
            ...
            PyEntidadHLP.executeTransaction(sqls, "Error", "Error", "",DatabaseConstant.conexionDefault); 
