...
@WebServlet(name = "CategoriesServiceImg", urlPatterns = {"/CategoriesServiceImg"})  
public class CategoriesServiceImg extends HttpServlet {   
    ...
    public static String guardar(HttpServletRequest request,HttpServletResponse response){ 
         ...
         try { 
            PyVO p = new PyVO(CategoriesService.str.cantCols); 
              
            List items = upload.parseRequest(request); 
            Iterator iterator = items.iterator(); 
             
            String fromABMCategories =""; 
            ...
            while (iterator.hasNext()){ 
                 ...
                 if (item.isFormField()) { 
                    ...
                    if (item.getFieldName().equals("fromABMCategories")){ 
                       fromABMCategories = item.getString(); 
                    }
                    else if (item.getFieldName().equals(CategoriesService.str.id)){/****/ 
                        p.setDecodedValue(CategoriesService.str.id ,item.getString(), encode); 
                    } 
                    else if (item.getFieldName().equals(CategoriesService.str.nombre)){/****/ 
                        p.setDecodedValue(CategoriesService.str.nombre ,item.getString(), encode); 
                    } 
           ...
           if(fromABMCategories.equals("n")){  
               sqls.add(PyEntidadHLP.SQLInsert(p,CategoriesService.str)); 
           }else{  
               List<String> where = new ArrayList<String>(); 
               where.add(  p.getWhereFieldValue(CategoriesService.str, CategoriesService.str.id, p.getValue(CategoriesService.str.id)));                
               sqls.add(PyEntidadHLP.sqlUpdate(p,CategoriesService.str,where)); 
