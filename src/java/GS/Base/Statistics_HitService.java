...
@WebServlet(name = "Statistics_HitService", urlPatterns = {"/Statistics_HitService"}) 
public class Statistics_HitService extends HttpServlet { 
 
    public static Statistics_HitI str = new Statistics_HitI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accStatistics_Hit").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accStatistics_Hit").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accStatistics_Hit").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accStatistics_Hit").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de Statistics_Hit " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMStatistics_Hit").equals("n")){ 
               sqls.add(PyEntidadHLP.SQLInsert(nuevoDB,str));
           }else{ 
               List<String> where = new ArrayList<String>();
               where.add(  nuevoDB.getWhereFieldValue(str, str.idHit, nuevoDB.getValue(str.idHit)));
               where.add(  nuevoDB.getWhereFieldValue(str, str.idStatistic, nuevoDB.getValue(str.idStatistic)));
               sqls.add(PyEntidadHLP.sqlUpdate(nuevoDB,str,where));
           } 
 
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de Statistics_Hit", "Statistics_Hit", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
