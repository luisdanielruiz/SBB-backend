...
@WebServlet(name = "UserAppService", urlPatterns = {"/UserAppService"}) 
public class UserAppService extends HttpServlet { 
 
    public static UserAppI str = new UserAppI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accUserApp").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accUserApp").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accUserApp").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accUserApp").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de UserApp " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMUserApp").equals("n")){ 
               sqls.add(PyEntidadHLP.SQLInsert(nuevoDB,str));
           }else{ 
               List<String> where = new ArrayList<String>();
               where.add(  nuevoDB.getWhereFieldValue(str, str.idUser, nuevoDB.getValue(str.idUser)));  
               sqls.add(PyEntidadHLP.sqlUpdate(nuevoDB,str,where));
           } 
 
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de UserApp", "UserApp", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
