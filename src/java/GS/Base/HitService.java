...
@WebServlet(name = "HitService", urlPatterns = {"/HitService"}) 
public class HitService extends HttpServlet { 
 
    public static HitI str = new HitI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accHit").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accHit").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accHit").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accHit").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de Hit " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMHit").equals("n")){ 
            ...
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de Hit", "Hit", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
