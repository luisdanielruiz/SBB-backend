...
@WebServlet(name = "MovimientosServiceImg", urlPatterns = {"/MovimientosServiceImg"}) 
public class TesteoServiceImg extends HttpServlet {  
    ...
    public static String guardar(HttpServletRequest request,HttpServletResponse response){
         ...
         try {
            PyVO p = new PyVO(TesteoService.str.cantCols);
             
            List items = upload.parseRequest(request);
            Iterator iterator = items.iterator();
            
            String fromABMMovimientos ="";
            ...
            while (iterator.hasNext()){
                 ...
                 if (item.isFormField()) {
                    ...
                    if (item.getFieldName().equals("fromABMMovimientos")){
                        fromABMMovimientos = item.getString();
                    }else if (item.getFieldName().equals(TesteoService.str.apellido)){/****/
                        p.setDecodedValue(TesteoService.str.apellido ,item.getString(), encode);
                    }
           ...
           if(fromABMMovimientos.equals("n")){ 
               sqls.add(PyEntidadHLP.SQLInsert(p,TesteoService.str));
           }else{ 
               List<String> where = new ArrayList<String>();
               where.add(  p.getWhereFieldValue(TesteoService.str, TesteoService.str.idTest, p.getValue(TesteoService.str.idTest)));               
               sqls.add(PyEntidadHLP.sqlUpdate(p,TesteoService.str,where));
