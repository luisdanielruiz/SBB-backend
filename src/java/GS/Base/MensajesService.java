...
@WebServlet(name = "MensajesService", urlPatterns = {"/MensajesService"}) 
public class MensajesService extends HttpServlet { 
 
    public static MensajesI str = new MensajesI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accMensajes").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accMensajes").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accMensajes").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accMensajes").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de Mensajes " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMMensajes").equals("n")){
               sqls.add(PyEntidadHLP.SQLInsert(nuevoDB,str));
           }else{
               List<String> where = new ArrayList<String>();
               where.add(  nuevoDB.getWhereFieldValue(str, str.idMensaje, nuevoDB.getValue(str.idMensaje)));  
               sqls.add(PyEntidadHLP.sqlUpdate(nuevoDB,str,where));
           } 
 
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de Mensajes", "Mensajes", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
