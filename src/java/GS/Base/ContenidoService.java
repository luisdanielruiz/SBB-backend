...
@WebServlet(name = "ContenidoService", urlPatterns = {"/ContenidoService"}) 
public class ContenidoService extends HttpServlet { 
 
    public static ContenidoI str = new ContenidoI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accContenido").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accContenido").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accContenido").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accContenido").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de Contenido " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMContenido").equals("n")){ 
            ...
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de Contenido", "Contenido", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
