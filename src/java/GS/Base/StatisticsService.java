...
@WebServlet(name = "StatisticsService", urlPatterns = {"/StatisticsService"}) 
public class StatisticsService extends HttpServlet { 
 
    public static StatisticsI str = new StatisticsI();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) 
        ...
        try { 
            if (UsuarioService.comprobarSession(request)){
               if (request.getParameter("accStatistics").equals("guardar")){ 
                   out.println(guardar(request)); 
               }if (request.getParameter("accStatistics").equals("eliminar")){ 
                   out.println(eliminar(request)); 
               }else if (request.getParameter("accStatistics").equals("getData")){ 
                   out.println(getData(request)); 
               }else if (request.getParameter("accStatistics").equals("getDataT")){ 
    ...
    public static String guardar(HttpServletRequest request){ 
        try{ 
           ...
           //sqls.add(LogService.loguearSQL(new Date(), usuario.usuario, "Alta de Statistics " ,request.getRemoteAddr()));
           if(request.getParameter("fromABMStatistics").equals("n")){ 
            ...
            PyEntidadHLP.executeTransaction(sqls, "Error en alta de Statistics", "Statistics", "",DatabaseConstant.conexionDefault);
            return "OK#1231321";             
        }catch (Exception exe){ 
            return "ER"; 
        } 
